...
pub const TrackSearchResult = struct {
    ...
    pub fn make_request(allocator: std.mem.Allocator, tokener: *SerializedToken, track_name: []const u8, album_name: ?[]const u8, artist_name: ?[]const u8, result_count: u8, logs: *std.fs.File, errlock: *std.Thread.Mutex) !TrackSearchResult {
        ...
        _ = switch (request.status) {
            ...
            std.http.Status.too_many_requests => {
                std.debug.print("Demasiadas requests, BORRAR dump.a\n", .{});
                try tokener.update();
            },
